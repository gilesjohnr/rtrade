get_sell_logic <- function(d, t, param, live=TRUE, trades=NULL, verbose=TRUE) {
  ...
  if (!is.na(d$bb_slope[t]) & d$ema_short_slope[t] < (param$slope_threshold_bband_mode + 0.001) & d$ema_short_slope[t] > (-1*param$slope_threshold_bband_mode - 0.001) ) {

    # SIDEWAYS TREND

    logic_sell <- d$mid[t] > d$bb_hi[t]
    if (logic_sell & verbose) message(":: Logic SELL (bbdand mode) ::")
