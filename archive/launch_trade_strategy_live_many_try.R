...
while (TRUE) {
  ...
  tryCatch({
    ...
    param$sell_trigger_low <- -1*max(d$atr[sel]/d$mean[sel], na.rm=T) * 1.01
    param$sell_trigger_high <- -1*param$sell_trigger_low * 3 # Risk ratio of 3:1
    ...
    if (F) {
      ...
      lines(d$date_time, d$supertrend, type='l', lwd=2, col='purple', ylim=range(c(d$supertrend, d$mean), na.rm=T), main=param$symbol)
    ...
    if (bal_usd > bal_sym & bal_usd > 1 | last_order$side == 'SELL') {
      ...
      if (logic_buy) {
        ...
        tryCatch({
          ...
          candidates <- tmp[tmp$ecdf < param$bid_ask_ecdf_threshold,]
          ...
          if (tmp$status != "FILLED") { # TRY 2
            ...
            if (tmp$status != "FILLED") { # TRY 3
              ...
              Sys.sleep(param$sleep_bt_orders+15)
              tmp <- get_order(param$symbol, buy_order$orderId)
      ...
      logic_sell_4 <- current_price > d$bb_hi[i] & d$supertrend_long_delta[i] < param$slope_threshold
      ...
      if (logic_sell) {
        ...
        tryCatch({
          ...
          sell_order <- create_order(symbol = param$symbol,
                                     ...
                                     type = 'LIMIT',
                                     price = ask,
                                     quantity = round(bal$free[bal$asset == param$asset] - 1e-06, 5),
                                     time_in_force = 'GTC',
                                     time_window = 10000)
